...
//                e.printStackTrace(); anti-pattern
    ...
    private static void actOnCell(String cellInput, String userActionIInput) {
    ...
    private static void checkIfGameIsOver() {
        boolean isAllChecked = isAllCellChecked(); // is -> returns boolean
        checkGameOver(isAllChecked); // check -> void
    }

    private static void checkGameOver(boolean isAllChecked) {
        if (isAllChecked) {
    ...
    private static void showBoard() {
        ...
        for (int row = 0; row < BOARD_ROW_SIZE; row++) {
            ...
            for (int col = 0; col < BOARD_COL_SIZE; col++) {
                System.out.print(BOARD[row][col].getSign() + " "); // 프린트 역할을 생각해서 getter 사용
            }
            System.out.println();
        }
        System.out.println();
