...
public class OutputHandler {
    ...
    public void showPassListForSelection(List<StudyCafeSeatPass> passes) {
        ...
        for (int index = 0; index < passes.size(); index++) {
            ...
            System.out.println(String.format("%s. ", index + 1) + this.display(pass));
        }
    }
    ...
    public void showPassOrderSummary(StudyCafeSeatPass selectedPass, StudyCafeLockerPass lockerPass) {
        ...
        System.out.println("이용권: " + this.display(selectedPass));
        if (lockerPass != null) {
            System.out.println("사물함: " + this.display(lockerPass));
        }

        double discountRate = selectedPass.getCharge().getDiscountRate();
        int discountPrice = (int) (selectedPass.getCharge().getPrice() * discountRate);
        if (discountPrice > 0) {
            System.out.println("이벤트 할인 금액: " + discountPrice + "원");
        }

        int totalPrice = selectedPass.getCharge().getPrice() - discountPrice + (lockerPass != null ? lockerPass.getCharge().getPrice() : 0);
    ...
    public String display(StudyCafePass pass) {
        Charge charge = pass.getCharge();
        StudyCafePassType passType = pass.getPassType();

        int price = charge.getPrice();
        int duration = charge.getDuration();
